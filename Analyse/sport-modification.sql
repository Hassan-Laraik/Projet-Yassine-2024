...
CREATE TABLE IF NOT EXISTS `depenses` (
  `irdredepenses` INT NOT NULL AUTO_INCREMENT,
  `montant` FLOAT(10,2) NOT NULL,
  `benificiaire` VARCHAR(200) NOT NULL,
  `document` VARCHAR(45) NOT NULL,
  `numero_document` VARCHAR(45) NOT NULL,
  `date_paiement` DATE NOT NULL,
  `observations` VARCHAR(200) NULL,
  PRIMARY KEY (`irdredepenses`))
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Placeholder table for view `credit_adhesion_sport`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `credit_adhesion_sport` (`cin` INT, `idsport` INT, `somme` INT);
...
-- -----------------------------------------------------
-- View `credit_adhesion_sport`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `credit_adhesion_sport`;
DROP VIEW IF EXISTS `credit_adhesion_sport` ;
USE `salle_sport3`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `credit_adhesion_sport` AS select `paiements`.`cin` AS `cin`,`paiements`.`idsport` AS `idsport`,coalesce(sum(`paiements`.`montant`),0) AS `somme` from `paiements` group by `paiements`.`cin`,`paiements`.`idsport`;
